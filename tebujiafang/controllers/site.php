...
class Site extends IController
{
	...
	function products()
	{
		...
		$queryEx   = new IModel("category_extend");
		$queryList = $queryEx->query("category_id=".$goods_info['category'],'goods_id');
		...
		$this->nextId = $queryList[$nextKey]['goods_id'];
		$this->lastId = $queryList[$lastKey]['goods_id'];
...
/*  格式化所有商品的货号，随机生成6位数，包括商品表和规格表 2016.5.20  */
		// $goodsDB = new IModel('goods');
		// $data = $goodsDB->query('id!=0','id,goods_no');
		// foreach ($data as $key => $value) {
		// 	  $code = goods_class::createGoodsNo();  // 获取一个不存在的商品货号
		// 		$goods_id   = $value['id'];
		// 		$productsDB = new IModel('products');
		// 		$proData = $productsDB->query('goods_id = '.$goods_id,'id,products_no');
		// 		foreach ($proData as $k => $val) {
		// 				$proUp['products_no'] = (count($proData) > 1) ? $code."-".$k : $code; 
		// 				$productsDB->setData($proUp);
		// 				$productsDB->update('id = '.$val['id']);
		// 		}
		// 		$goodsUp['goods_no'] = (count($proData) > 1) ? $code.'-0' : $code;
		// 		$goodsDB->setData($goodsUp);
		// 		$goodsDB->update('id = '.$goods_id);
		// }	
		
/*  剔除goods_photo图片库里的重复数据 */
		$photoDB = new IModel('goods_photo');
		$data = $photoDB->query();
		foreach ($data as $key => $value) {
			$newArray[$value['img']] = $value['id'];
		}
		$tmpArray  = array_unique($newArray);

echo count($newArray);
echo "|||";
echo count($tmpArray);

		$repeatArr = array_diff_assoc($newArray,$tmpArray);
		foreach ($repeatArr as $key => $value) {
			$photoDB->del('id = "'.$value.'" AND img = "'.$key.'"');
		}
	
/*  剔除goods_photo图片库里在关联表里不存在的垃圾数据/goods_photo_relation关联库里不存在图片的垃圾关联 */		
		// $photoDB = new IModel('goods_photo');
		// $data = $photoDB->query();
		// foreach ($data as $key => $value) {
		// 	$newArray[$value['img']] = $value['id'];
		...
		// $relationDB = new IModel('goods_photo_relation');
		// $data2 = $relationDB->query();
		// foreach ($data2 as $key => $value) {
		// 	$newArray2[$value['id']] = $value['photo_id'];
		// }

		// $repeatArr1 = array_diff($newArray,$newArray2);  // 筛选删除图片库里的垃圾图片 goods_photo
		// foreach ($repeatArr1 as $key => $value) { 
		// 	$photoDB->del('id = "'.$value.'" AND img = "'.$key.'"');
		// }

		// $repeatArr2 = array_diff($newArray2,$newArray);  // 筛选删除关联库里的无用关联  goods_photo_relation
		// foreach ($repeatArr2 as $key => $value) { 
		// 	$relationDB->del('id = "'.$key.'" AND photo_id = "'.$value.'"');
		// }
