...
class Member extends IController
{
	...
	public function seller2_add()
	{
		...
		{
			$this->sellerRow = $_POST;
			$this->redirect('seller2_edit',false);
			Util::showMessage($errorMsg);
		}
	 ...
	 * @brief 代理商的回收站删除动作
	 */
	public function seller2_recycle_del()
	{
		$id = IFilter::act(IReq::get('id'),'int');
		$sellerDB = new IModel('seller');
		$goodsDB  = new IModel('goods');
		...
		$this->redirect('seller2_recycle_list');
	}
	/**
	 * @brief 商户的回收站恢复动作
	 */
	 ...
	 * @brief 代理商的回收站恢复动作
	 */
	public function seller2_recycle_restore()
	{
		$id = IFilter::act(IReq::get('id'),'int');
		$sellerDB = new IModel('seller');
		$data = array('is_del' => 0);
	...
	public function seller2_copy() {

		$idArray   = IReq::get('goods_id');
		$numsArray = IReq::get('goods_nums');  // 数量数组
		$idString  = implode(IReq::get('goods_id'), ',');
		$seller_id = IReq::get('seller_id');
		if (!$idString) { 
			$this->redirect('seller2_goods_edit?id='.$seller_id);
		}

		foreach ($numsArray as $key => $value) {
			$goodsNums[$idArray[$key]] = $value;
		}
		...
		// 查看复制的商品存在不存在
		$goodsData = $goodsDB->query('id in ('.$idString.') and is_del = 0 and seller_id = 0','*');
		...
		if($goodsData)
		{
			...
			{
				//判断是否重复,重复就跳过
				if( $goodsDB->getObj('seller_id = '.$seller_id.' and name = "'.$val['name'].'"') ) { continue; }
				...
				$val['goods_no']  .= '-'.$seller_id;
				$val['store_nums'] = $goodsNums[$oldId];

				$goodsDB->setData($val);
				$goods_id = $goodsDB->add();

				//商品分类 echoshiki 20160516
				$categoryExtend = new IModel('category_extend');
				$cateData = $categoryExtend->query('goods_id = '.$oldId);
				foreach ($cateData as $key => $value) {
					$newCateData['goods_id'] = $goods_id;
					$newCateData['category_id'] = $value['category_id'];
					$categoryExtend->setData($newCateData);
					$categoryExtend->add();
					unset($newCateData);
				}
				...
				if($productsData)
				{
					...
					{
						unset($v['id']);
						$v['products_no'].= '-'.$seller_id;
						$v['goods_id']    = $goods_id;
						$v['store_nums']  = $val['store_nums'];
						$productsDB->setData($v);
						$productsDB->add();
					}
				}

				$countAll = count($productsData);  //累计总数

				$update_nums['store_nums'] = $countAll * $val['store_nums'];  //更新主表库存数据 = 规格库存的总和
				$goodsDB->setData($update_nums);
				$goods_id = $goodsDB->update('id = '.$goods_id);

			}
			$this->redirect('/index.php?controller=member&action=seller2_goods_edit&id='.$seller_id);
		}
		else
		{
			die('复制的商品不存在');
	...
	public function seller2_goods_edit() {
    	$data = array();
    	//获得seller_id的值
		$seller_id = IFilter::act(IReq::get('id'),'int');
		...
		if($seller_id)
		{
			...
			$goodsObj->fields  = "go.id,go.name,go.goods_no,go.sell_price,go.store_nums,go.spec_array";
			$goodsObj->page    = $page;
			$goodsObj->where   = "go.seller_id = '".$seller_id."'";
			
			$this->goodsObj   = $goodsObj;
			$this->goodsInfo  = $goodsObj->find();
		}
		$this->redirect('seller2_goods_edit');
    }
