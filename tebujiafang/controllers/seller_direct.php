 ...
 */
class Seller_direct extends IController
{
	public $layout = 'seller_direct';
	...
	public function ajax_sort()
	{
		...
		$goodsDB->update("id = {$id} and seller_id = ".$this->seller_direct['seller_id']);
	}
	...
	public function order_show()
	{
		...
		if($order_id)
		{
			...
			$data = $order_show->getOrderShow($order_id,0,$this->seller_direct['seller_id']);
			if($data)
			{
		 		//获取地区
	...
	{
		$seller_id = $this->seller_direct['seller_id'];
	...
	public function bill_edit()
	{
		...
		if($id)
		{
			...
			$billRow = $billDB->getObj('id = '.$id.' and seller_id = '.$this->seller_direct['seller_id']);
		}
	...
	public function bill_del()
	{
		...
		{
			$billDB = new IModel('bill');
			$billDB->del('id = '.$id.' and seller_id = '.$this->seller_direct['seller_id'].' and is_pay = 0');
		}
	...
	public function bill_update()
	{
		...
		if($id)
		{
			...
			if($billRow['is_pay'] == 0)
			{
				...
				$billDB->update('id = '.$id.' and seller_id = '.$this->seller_direct['seller_id']);
			}
		}
		else
		{
			//判断是否存在未处理的申请
			$isSubmitBill = $billDB->getObj(" seller_id = ".$this->seller_direct['seller_id']." and is_pay = 0");
			if($isSubmitBill)
			{
			...
			//获取未结算的订单
			$queryObject = CountSum::getSellerDirectGoodsFeeQuery($this->seller_direct['seller_id'],$start_time,$end_time,0);
			...
			if($countData['countFee'] > 0)
			{
				...
				$site_config = $siteConfig->getInfo();

				$billString = AccountLog::sellerDirectBillTemplate($countData,$site_config);
				$data = array(
					'seller_id'  => $this->seller_direct['seller_id'],
	...
	public function countGoodsFee()
	{
		...
		$queryObject = CountSum::getSellerDirectGoodsFeeQuery($seller_id,$start_time,$end_time,0);
		$countData   = CountSum::countSellerOrderFee($queryObject->find());
	...
	public function getSellerName($id){
		...
		$sellerRow = $sellerObj->getObj('id ='.$id,'seller_name');

		return $sellerRow['seller_name'];
	}
	 ...
	 *	获取直销商订单提成的代理商的名字
	 * */
	public function getOrderSellerName($id){

		$OrderObj = new IModel('order');

		$OrderRow = $OrderObj->getObj('id ='.$id,'seller_id');

		$SellerObj= new IModel('seller');

		$SellerRow = $SellerObj->getObj('id ='.$OrderRow['seller_id'],'seller_name');
	...
	/**
	 * 代理商的商品种类
	 * WJAZZ 2016-03-30 15:57:03
	 * @param
	 * @return
	 */
	public function goods_num($id)
	{
		$goodsObj = new IModel('goods');

		$data = $goodsObj->getObj('seller_id ='.$id,'count(*) as num');
	...
	/**
	 * 代理商数量
	 * WJAZZ 2016-03-30 16:44:11
	 * @param
	 * @return 
	 */
	public function sellers_num($id)
	{
		$sellersObj = new IModel('seller');

		$data = $sellersObj->getObj('parent_id ='.$id,'count(*) as num');
	...
	/**
	 * 可提现订单数和总金额
	 * WJAZZ 2016-04-01 15:34:29
	 * @param
	 * @return
	 */
	public function agent_order_num($id)
	{
		$sellersObj = new IModel('commission');

		$data = $sellersObj->getObj('sellerid ='.$id.' and status<2 ','count(*) as num,sum(balance) as moneyall');
	 ...
	 * @return
	 */
	public function agented_order_num($id)
	{
		$sellersObj = new IModel('commission');

		$data = $sellersObj->getObj('sellerid ='.$id.' and status=2 ','sum(balance) as moneyall');

		if(!$data['moneyall']){
			return 0;
		}

		return $data['moneyall'];
	}
}