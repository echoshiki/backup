 ...
 * @note
 */
class Simple_agent extends IController
{
    public $layout='site_mini_agent';
    ...
    function cart2()
    {
        ...
        // 2016/04/21 echoshiki 代理商客户跳过登陆直接到结算页面
    	//必须为登录用户
    ...
    function cart3()
    {
    	$accept_name   = IFilter::act("代理商客户");
    	$province      = IFilter::act("110000",'int');
    	$city          = IFilter::act("110100",'int');
    	$area          = IFilter::act("110101",'int');
    	$address       = IFilter::act("代理商地址");
    	$mobile        = IFilter::act("13000000000");
    	$telphone      = IFilter::act("100000");
    	$zip           = IFilter::act("100000");
    	$delivery_id   = IFilter::act(1,'int');
    	$accept_time   = IFilter::act("任意");
