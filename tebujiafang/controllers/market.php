	...
	public function bill_update()
	{
		...
		if($id)
		{
			...
			if(isset($billRow['order_ids']) && $billRow['order_ids'])
			{
				...
				//提成信息表状态更新 wj 2016年04月06日17:16:50
				$commissionDB=new IModel('commission');
				...
				foreach($orderIdArray as $key => $val)
				{
					...
					$commissionDB->setData(array('status' => 2));
					$commissionDB->update('orderid = '.$val.' and sellerid='.$billRow['seller_id']);
				}
			}
