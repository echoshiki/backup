...
class Order extends IController
{
    ...
    {
    	// print_r($_POST); exit();
    	//获取整理后的discount,公式 新价格*新数目 - 旧价格*新数目 = discount
			$newPriceArr = $_POST['newPrice'];    	
			$oldPriceArr = $_POST['oldPrice'];  
			$numGoods    = $_POST['goods_nums'];
			$goodsIdArr  = $_POST['goods_id'];
			$order_id    = IFilter::act(IReq::get('id'),'int');


			foreach ($numGoods as $key => $value) {
				 $newTotal += $newPriceArr[$key]*$numGoods[$key];
				 $oldTotal += $oldPriceArr[$key]*$numGoods[$key];
			}
			$discountReal = $newTotal- $oldTotal;
    	...
    	$dataArray['discount']      = $order_id ? $discountReal : 0;
