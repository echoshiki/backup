...
class CheckRights extends IInterceptorBase
{
	...
	public static function checkSellerRights()
	{
		...
		$type_id	  =	ISafe::get('seller_type_id');
		if($type_id!=2){
			$object->redirect('/systemseller/index');
		}

		//1,针对独立配置的action检测
		if(isset(self::$sellerAction[$controllerId."@".$actionId]) && method_exists(__CLASS__,self::$sellerAction[$controllerId."@".$actionId]) && $type_id==2)
		{
	 ...
	 * @return boolean
	 */
	public static function checkSellerDirectRights()
	{
		$object       = IWeb::$app->getController();
		$controllerId = $object->getId();
		$actionId     = $object->getAction()->getId();
		...
		//1,针对独立配置的action检测
		if(isset(self::$sellerAction[$controllerId."@".$actionId]) && method_exists(__CLASS__,self::$sellerAction[$controllerId."@".$actionId]) && $type_id == 1)
		{
			call_user_func(array(__CLASS__,self::$sellerAction[$controllerId."@".$actionId]));
			return;
		}
		...
		{
			$object->seller_direct = self::getSeller();
			if(!$object->seller_direct)
			{
				$object->redirect('/systemseller/index');
			}
		}
