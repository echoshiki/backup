...
class search_goods
{
	...
	public static function find($defaultWhere = '',$limit = 21,$isCondition = true)
	{
			...
			else if(is_array($defaultWhere))
			{
				foreach($defaultWhere as $key => $val)
				{
					...
					if($key == 'category_extend')
					{
						...
						if (IReq::get('new_cat')=="1") {
							// 调出所有产品
							$categoryExtendList = $categoryExtendObj->query("category_id != 0",'goods_id','id','desc');
						}else{
		...
		//新增 热卖/促销/推荐/新出 商品筛选条件
		if(IReq::get('commend_id')){
			$goodsObj->join = "left join commend_goods AS co ON go.id = co.goods_id";
			$where .= ' AND co.commend_id='.IReq::get('commend_id');
		}
