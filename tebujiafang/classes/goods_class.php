...
class goods_class
{
	...
	public static function createGoodsNo()
	{
		...
		$data = $goodsDB->getObj('goods_no = "'.$code.'"');  // 查询商品总表
		if ($data) {  //检测是否存在
			self::createGoodsNo();
		} else {
			return $code;
	...
	{
		// print_r($paramData['agent_group_id']);exit();
		$postData = array();
		...
		if(isset($postData['_spec_array']))
		{
			...
			// $oldData = $postData['_spec_array'];

			// unset($postData['_spec_array']);
			// krsort($oldData);
			// foreach ($oldData as $key => $value) {
			// 	$postData['_spec_array'][] = $value;
			// }
		...
		//处理商品与代理商组关系表
		$agentgroupDB = new IModel('agentgroup_extend');
		$agentgroupDB->del('goods_id = '.$id);
		
			if($paramData['agent_group_id']){
				$agentgroupDB->setData(array('goods_id' => $id,'agent_group_id' => $paramData['agent_group_id']));
				$agentgroupDB->add();
			}
	...
	*/
	...
	public function edit($id)
	{
		...
		$product_info = $productObj->query('goods_id = '.$id.' ORDER BY id asc ');
		if($product_info)
		{
			//获取货品会员价格
