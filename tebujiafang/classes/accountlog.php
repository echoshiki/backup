...
class AccountLog
{
	 ...
	 */
	public static function sellerBillTemplate($countData = null,$site_config)
	{
		$replaceData = array(
			...
			'{agent}'			 => $site_config['rebates_seller']*$countData['orderAmountPrice']/100
		);

		$templateString = "结算起止时间：【{startTime}】到【{endTime}】，订单号：【{orderNoList}】，订单数量共计：【{orderNum}单】，结算金额计算明细：【订单总金额：￥{orderAmountPrice}】-【提成总金额：￥{agent}】";
		return strtr($templateString,$replaceData);
	}

	/**
	 * wj 2016年04月05日11:42:12
	 * @brief 直销商结算单模板
	 * @param array $countData 替换的数据
	 */
	public static function sellerDirectBillTemplate($countData = null,$site_config)
	{
		$replaceData = array(
			'{startTime}'        => $countData['start_time'],
			'{endTime}'          => $countData['end_time'],
			...
			'{commissionPer}'    => $countData['commissionPer'],
			'{commission}'       => $countData['commission'],
			'{agent}'			 => $site_config['rebates_direct']*$countData['orderAmountPrice']/100
		);
