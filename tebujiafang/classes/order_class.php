...
class Order_Class
{
	...
	public static function sendDeliveryGoods($order_id,$order_goods_relation,$sendor_id,$sendor = 'admin')
	{
		...
		$sendStatus = 2;//部分发货
	 	...
	 	//更新发货状态
	 	//游客去代理商门店购买情况
		if($tbOrderRow['user_id']==0 and $tbOrderRow['seller_id']!=0){
		...
		if($tbOrderRow['user_id']==0 and $tbOrderRow['seller_id']!=0){
			...
			$rebates_seller = $siteConfig->rebates_seller;
			$rebates_direct = $siteConfig->rebates_direct;
			$order_amount = $tbOrderRow['order_amount'];//总金额
			$rebates_seller = $order_amount*$rebates_seller/100;
			$rebates_direct = $order_amount*$rebates_direct/100;

			$tb_commission = new IModel('commission');
			$tb_commission->setData(array(
    		'orderid' => $order_id,
    		'sellerid'     => $tbOrderRow['seller_id'],
    		'type'   => 2,
    		'status'   => 0,
    		'balance'     => $rebates_seller,
    		'created_at'  => date('Y-m-d H:i:s'),
    		'updated_at'  => date('Y-m-d H:i:s')
    	));
			$insertResult = $tb_commission->add();


			$sellerObj = new IModel('seller');
			$sellerRow = $sellerObj->getObj("id = ".$tbOrderRow['seller_id']);

			$tb_commission2 = new IModel('commission');
			$tb_commission2->setData(array(
    		'orderid' => $order_id,
    		'sellerid'     => $sellerRow['parent_id'],
    		'type'   => 1,
    		'status'   => 0,
    		'balance'     => $rebates_direct,
    		'created_at'  => date('Y-m-d H:i:s'),
    		'updated_at'  => date('Y-m-d H:i:s')
    	));
			$insertResult2 = $tb_commission2->add();
