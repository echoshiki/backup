...
class statistics
{
	...
	/**
	 * 直销商提成统计
	 * WJAZZ 2016-04-05 17:39:24
	 * @param
	 * @return
	 */
	public static function sellerDirectAgentSum($id,$start,$end)
	{
		$db=new IQuery("commission");

		$db->fields="balance as yValue,`created_at`";
		$db->where="type=1 and sellerid=".$id;


//		$data=$db->find();
//		$chart=array();
//		foreach($data as $v){
//			$chart[$v['created_at']]=floor($v['balance']);
//		}
//		print_r($db->find());
		$data= self::ParseCondition($db,'created_at',$start,$end);
