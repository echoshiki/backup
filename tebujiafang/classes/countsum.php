...
class CountSum
{
    ...
    public static function getSellerGoodsFeeQuery($seller_id = '',$start_time = '',$end_time = '',$is_checkout = '')
    {
    	...
    	$where .= $seller_id <> 0         ? " and seller_id = ".$seller_id : "  and seller_id <>0 ";
	 ...
	 * @param IQuery 结果集对象
	 */
	public static function getSellerDirectGoodsFeeQuery($seller_id = '',$start_time = '',$end_time = '',$is_checkout = '')
	{
...
//		$items = $query->find();
		...
		$where = $seller_id ? 'co.sellerid ='.$seller_id : '';
		$where .= $start_time         ? " and created_at >= '{$start_time}' " : "";
		$where .= $end_time           ? " and created_at <= '{$end_time}' "   : "";
		$orderGoodsDB = new IQuery('commission as co');
		$orderGoodsDB->join ="left join order AS od ON co.orderid=od.id";
		$orderGoodsDB->fields ="co.*,od.order_no,od.order_amount,od.is_checkout";
		$orderGoodsDB->order = "co.id desc";
		$orderGoodsDB->where = $where;
		return $orderGoodsDB;
	}
