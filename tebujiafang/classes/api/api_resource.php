...
return array(
	...
	'getCommendRecom' => array(
		'query' => array(
			'name' => 'commend_goods as co',
			'join' => 'left join goods as go on co.goods_id = go.id ',
			'where' => 'co.commend_id = 4 and go.is_del = 0 AND go.id is not null and go.is_del = 0 ',
			'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price',
			'limit'=>'10',
			'order'=>'sort asc,id desc'
		)
	...
	//推荐商品列表
	'getCommendRecomEx' => array(
		'query' => array(
			'name' => 'commend_goods as co',
			'join' => 'left join goods as go on co.goods_id = go.id left join category_extend as ca on ca.goods_id = go.id ',
			'where' => 'co.commend_id = 4 and go.is_del = 0 AND go.id is not null AND ca.category_id = #categroy_id# and go.is_del = 0 ',
			'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price',
			'limit'=>'10',
			'order'=>'sort asc,id desc'
		)
