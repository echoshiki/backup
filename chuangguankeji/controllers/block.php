...
class Block extends IController
{
    ...
    */
	...
	/**
    * @brief 【重要】进行众筹支付的支付方法
    */
	public function doPayfund()
	{
		//获得相关参数
		$order_id   = IReq::get('order_id');
		// $recharge   = IReq::get('recharge');//充值金额
		$payment_id = IFilter::act(IReq::get('payment_id'),'int');
		...
		if($order_id)
		{
			...
			//获取订单信息
			$orderDB  = new IModel('fund_order');
			$orderRow = $orderDB->getObj('id = '.current($order_id));

			if(empty($orderRow))
			{
				IError::show(403,'要支付的众筹订单信息不存在');
			}
			$payment_id = $orderRow['pay_type'];
		}
		...
		if($order_id)
		{
			$sendData = $paymentInstance->getSendData(Payment::getPaymentInfofund($payment_id,'order',$order_id));
		}
		else
		{
			IError::show(403,'发生支付错误');
	...
	public function callback()
	{
		...
		if($return == 1)
		{
			...
			else
			{
				...
				{
					if($orderRow){//判断是否是众筹订单
					$order_id = Order_Class::updateOrderfundStatus($item);
					}else{
				...
				if($orderRow){//判断是否是众筹订单
				$this->redirect('/site/success/message/'.urlencode("支付成功").'/?callback=/ucenter/orderfund');
				}else{
