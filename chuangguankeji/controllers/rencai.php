 ...
 * @note  后台
 */
class Rencai extends IController
{
	public $layout='admin';
	...
	function rencai_del()
	{
		...
		if(!empty($id))
		{
			...
			$this->redirect('rencai_list');
		}
		else
		{
			$this->redirect('rencai_list',false);
			Util::showMessage('请选择要删除的人才');
		}
	}

	//[众筹]单页
	function rencai_edit()
	{
		$data = array();
		$id = IFilter::act(IReq::get('id'),'int');
		if($id)
		{
			...
			if($data)
			{
				...
				$this->redirect('rencai_edit',false);
			}
		}
		if(count($data)==0)
		{
			$this->redirect('rencai_edit');
		}

	}

	//[众筹]增加修改
	function rencai_edit_act()
	{
		...
		$DataArray  = array(
			...
			'content3'     => IFilter::act(IReq::get('content3','post'),'text'),
			'content4'     => IFilter::act(IReq::get('content4','post'),'text'),
		...
		if(isset($_FILES['ewm']['name']) && $_FILES['ewm']['name']!='')
		{
			$uploadObj = new PhotoUpload();
			$uploadObj->setIterance(false);
			$photoInfo = $uploadObj->run();
			if(isset($photoInfo['ewm']['img']) && file_exists($photoInfo['ewm']['img']))
			{
				$DataArray['ewm'] = $photoInfo['ewm']['img'];
			}
		}
	...
	function cat_edit_act()
	{
		...
		$catObj    = new IModel('rencai_category');
		$DataArray = array(
			'parent_id' => $parent_id,
		...
		if($id)
		{
			...
			if($isMoveNode == true)
			{
				if($parentRow['path']!=null && strpos($parentRow['path'],','.$id.',')!==false)
				{
					...
					$this->redirect('rencai_cat_edit',false);
					Util::showMessage('不能该节点移动到其子节点的位置上');
				}
				else
				{
					//其子节点批量移动
					$childObj = new IModel('rencai_category');
					$oldPath  = $catRow['path'];
	...
	function cat_del()
	{
		...
		if($isCheck == false)
		{
			...
			$this->redirect('rencai_cat_list',false);
