 ...
 * @note  后台
 */
class Project extends IController
{
	public $layout='admin';
	...
	function project_del()
	{
		...
		{
			$this->redirect('project_list',false);
			Util::showMessage('请选择要删除的项目');
		}
	}

	function project_feedback_del()
	{
		$id = IFilter::act( IReq::get('id') ,'int' );
		if(!empty($id))
		{
			$obj = new IModel('feedback');
			// $relationObj = new IModel('relation');
			...
			$obj->del($where1);               //删除商品
			// $relationObj->del($where2);       //删除关联商品表
			$this->redirect('project_feedback_list');
		}
		else
		{
			$this->redirect('project_feedback_list',false);
			Util::showMessage('请选择要删除的咨询信息');
		}
	}
	...
	function isvis()
	{
		...
		$keywordObj = new IModel('project');
		$dataArray  = array('visibility' => 2);
		$keywordObj->setData($dataArray);
		$is_result  = $keywordObj->update('id = "'.$id.'"','visibility');

		$keywordRow = $keywordObj->getObj('id = "'.$id.'"');
		if($is_result!==false)
		{
			$userDB = new IModel('user');
			$userRow = $userDB->getObj('id = "'.$keywordRow['seller_id'].'"');


			$id       = $keywordRow['seller_id'];
			$balance  = $keywordRow['t_amount'];
			$balance  = $balance*0.1;
    	$type     = 'recharge'; //操作类型 recharge充值,withdraw提现金
    	$even     = '';
    	$note     = '管理员['.$this->admin['admin_id'].']'.$this->admin['admin_name'].'给用户['.$keywordRow['seller_id'].']'.$userRow['username'].'项目提成，金额：'.$balance.'元';
    	...
    	echo JSON::encode(array('isError' => false,'hot' => $keywordRow['visibility']));
    }
    else
    {
    	echo JSON::encode(array('isError'=>true,'message'=>'更改失败'));
    }
}
...
function project_edit()
{
	...
	if($id)
	{
		...
		if($data)
		{
			...
			$this->redirect('project_edit',false);
		}
	}
	if(count($data)==0)
	{
		$this->redirect('project_edit');
	}
...
function project_edit_act()
{
	...
	$DataArray  = array(
		...
		't_amount'    => IFilter::act(IReq::get('t_amount','post'),'float'),
	...
	if(isset($_FILES['pic']['name']) && $_FILES['pic']['name']!='')
	{
		...
		{
			$DataArray['pic'] = $photoInfo['pic']['img'];
		}
	}
...
function cat_edit_act()
{
	...
	$catObj    = new IModel('project_category');
	$DataArray = array(
		'parent_id' => $parent_id,
	...
	if($id)
	{
		...
		if($isMoveNode == true)
		{
			if($parentRow['path']!=null && strpos($parentRow['path'],','.$id.',')!==false)
			{
				...
				$this->redirect('project_cat_edit',false);
				Util::showMessage('不能该节点移动到其子节点的位置上');
			}
			else
			{
					//其子节点批量移动
				$childObj = new IModel('project_category');
				$oldPath  = $catRow['path'];
	...
	$this->redirect('project_cat_list');
}
...
function cat_del()
{
	...
	if($isCheck == false)
	{
		...
		$this->redirect('project_cat_list',false);
