...
class Seller extends IController
{
	...
	public function fund_edit()
	{
		...
		if($id)
		{
			...
			if($data)
			{
				...
				$this->redirect('fund_edit',false);
			}
		}
		if(count($data)==0)
		{
			$this->redirect('fund_edit');
		}
	...
	public function fund_del()
	{
		...
		{
			$idString = is_array($id) ? join(',',$id) : $id;
			$specObj  = new IModel('fund');
			$specObj->del("id in ( {$idString} ) and seller_id = ".$this->seller['seller_id']);
			$this->redirect('fund_list');
		}
		else
		{
			$this->redirect('fund_list',false);
			Util::showMessage('请选择要删除的众筹信息');
		}
	}
	//众筹订单删除
	public function fundorder_del()
	{
		$id = IFilter::act(IReq::get('id'),'int');
		$ulc = IReq::get('ulc');
		$ulc = urldecode($ulc);
    	...
    	//生成order对象
    	$tb_order = new IModel('fund_order');
    	...
    	if($id)
    	{
    		...
    		if(is_array($id))
		{
			$tb_order->update('id in ('.join(",",$id).')');
		}
		else
		{
			$tb_order->update('id = '.$id);
		}
			...
			// echo $url;exit();
			header('Location:'.$ulc);
    		// $this->redirect('fund_order');
    	}
    	else
    	{
    		$this->redirect('fund_order',true);
	...
	//众筹结算单修改
	public function bill_fund_edit()
	{
		$id = IFilter::act(IReq::get('id'),'int');
		$billRow = array();
		...
		if($id)
		{
			$billDB  = new IModel('bill_fund');
			$billRow = $billDB->getObj('id = '.$id.' and seller_id = '.$this->seller['seller_id']);
		}

		$this->billRow = $billRow;
		$this->redirect('bill_fund_edit');
	}
	...
	//众筹结算单更新
	public function bill_fund_update()
	{
		$id            = IFilter::act(IReq::get('id'),'int');
		$start_time    = IFilter::act(IReq::get('start_time'));
		$end_time      = IFilter::act(IReq::get('end_time'));
		...
		else
		{
			...
			if($isSubmitBill)
			{
				$this->redirect('bill_fund_list',false);
				Util::showMessage('请耐心等待管理员结算后才能再次提交申请');
			}

			//获取未结算的订单
			$queryObject = CountSum::getSellerFundFeeQuery($this->seller['seller_id'],$start_time,$end_time,0);
			$countData   = CountSum::countSellerOrderFee($queryObject->find());
			...
			if($countData['orderAmountPrice'] > 0)
			{
				...
				$billString = AccountLog::sellerfundBillTemplate($countData);
				$data = array(
					'seller_id'  => $this->seller['seller_id'],
					'apply_time' => date('Y-m-d H:i:s'),
					'apply_content' => IFilter::act(IReq::get('apply_content')),
	...
	//计算应该结算的众筹货款明细
	public function countFundFee()
	{
		$seller_id   = $this->seller['seller_id'];
		$start_time  = IFilter::act(IReq::get('start_time'));
		$end_time    = IFilter::act(IReq::get('end_time'));

		$queryObject = CountSum::getSellerFundFeeQuery($seller_id,$start_time,$end_time,0);
