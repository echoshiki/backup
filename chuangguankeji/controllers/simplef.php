 ...
 * @note
 */
class Simplef extends IController
{
    public $layout='site';
    ...
    function cart3()
    {
    	...
    	$order_amount  = IFilter::act(IReq::get('order_amount'),'float');
    	$order_amount1 = IFilter::act(IReq::get('order_amount1'),'float');
    	...
    	$fund_id       = IFilter::act(IReq::get('fund_id'),'int');
    	...
    	$user_id       = ($this->user['user_id'] == null) ? 0 : $this->user['user_id'];
            ...
            //查询订单数据库
            $orderObj  = new IModel('fund_order');
            $orderList = $orderObj->getObj("accept_name = '$accept_name' and mobile='$mobile' and NOW() < date_add(create_time,INTERVAL 2 MINUTE)");
            if($orderList){
                IError::show(403,'您所提交的订单重复，请稍候再试...');
            }
