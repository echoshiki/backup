...
class Ucenter extends IController
{
     ...
     * @brief 我的众筹订单列表
     */
    public function iorderfund()
    {
        $this->initPayment();
        $this->redirect('iorderfund');
    ...
    function project_edit_act()
    {
        ...
        $DataArray  = array(
            ...
            'seller_id'        => IFilter::act(IReq::get('seller_id','post'),'int'),
     ...
     * @return String
     */
    public function orderfund_detail()
    {
        $id = IFilter::act(IReq::get('id'),'int');
        ...
        $this->order_info = $tb_order->getObj($where);
        

        if(!$this->order_info)
        {
            IError::show(403,'众筹订单信息不存在');
        }
        $this->redirect('orderfund_detail',false);
    }
    ...
    //操作众筹订单状态
    public function orderfund_status()
    {
        $op    = IFilter::act(IReq::get('op'));
        $id    = IFilter::act( IReq::get('order_id'),'int' );
        $model = new IModel('fund_order');
        ...
        $this->redirect("orderfund_detail/id/$id");
    }
    /**
     * @brief 我的地址
     */
