 ...
 * @note  后台
 */
class Orderfund extends IController
{
    ...
    public function order_list()
    {
    	...
    	foreach($search as $k=>$val){
    		if($val=='accept_name' || $val=='order_no' || $val=='fund_id'){}else{
    		if($val!=''){
    			// if($i==1){
    			// 	if($k=='keywords'){
    			// 		$where .= $k." like '%$val%'";
    	...
    	if($search['name']!=''){
    		$where = str_replace("keywords",$search['name'],$where);
    	}
    	// echo $where;
		// $where = "pay_status = 0 and status ='' and accept_name =''";

		//拼接sql
    	$orderHandle = new IQuery('fund_order');
    	$orderHandle->order  = "id desc";
    ...
    public function order_recycle_list()
    {
