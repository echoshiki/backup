 ...
 * @note  后台
 */
class Fund extends IController
{
	public $layout='admin';
	...
	function pro_del()
	{
		...
		{
			$this->redirect('pro_list',false);
			Util::showMessage('请选择要删除的众筹');
		}
	}
	...
	function pro_edit()
	{
		...
		if($id)
		{
			...
			if($data)
			{
				...
				$this->redirect('pro_edit',false);
			}
		}
		if(count($data)==0)
		{
			$this->redirect('pro_edit');
		}

	}

	//[众筹]增加修改
	function pro_edit_act()
	{
	...
	function cat_edit_act()
	{
		...
		$catObj    = new IModel('fund_category');
		$DataArray = array(
			'parent_id' => $parent_id,
		...
		if($id)
		{
			...
			if($isMoveNode == true)
			{
				if($parentRow['path']!=null && strpos($parentRow['path'],','.$id.',')!==false)
				{
					...
					$this->redirect('fund_cat_edit',false);
					Util::showMessage('不能该节点移动到其子节点的位置上');
				}
				else
				{
					//其子节点批量移动
					$childObj = new IModel('fund_category');
					$oldPath  = $catRow['path'];
	...
	function cat_del()
	{
		...
		if($isCheck == false)
		{
			...
			$this->redirect('fund_cat_list',false);
