...
class Site extends IController
{
	...
	function feedback_act()
	{
    	...
    	{
    		$goodsObj = new IModel('project');
    		$goodsRow = $goodsObj->getObj('id = '.$project_id);
    		if(!$goodsRow)
    		{
    			$message = '不存在此项目';
    		}
    	...
    	{
    		// IError::show(403,$message);
    		die(JSON::encode(array('result' => 'fail','data' => $message)));
    	}
    	else
    	{
			$dataArray = array(
			...
			$referObj = new IModel('feedback');
			$referObj->setData($dataArray);
			$referObj->add();
			die(JSON::encode(array('result' => 'success','data' => $message)));
    	}
	}
	...
	function fund_detail()
	{
		...
		else
		{
			...
			if(empty($this->articleRow))
			{
				IError::show(403,'众筹项目不存在');
				exit;
			}

			
			$this->redirect('fund_detail');
		}
	}
	...
	//人才展示
	function rencai_detail()
	{
		// echo "string";exit();
		$rencai_id = IFilter::act(IReq::get('id'),'int');
		...
		//使用人才id获得人才信息
		$tb_rencai = new IModel('rencai');
		$rencai_info = $tb_rencai->getObj('id='.$rencai_id." AND visibility=1");
		if(!$rencai_info)
		{
			IError::show(403,"此人才不存在");
			exit;
		}

		// print_r($rencai_info);exit();

		

		$this->setRenderData($rencai_info);
		$this->redirect('rencai_detail');
	}
	//商品讨论更新
