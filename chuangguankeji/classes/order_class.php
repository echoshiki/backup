...
class Order_Class
{
	 ...
	 * @return false or int order_id
	 */
	public static function updateOrderfundStatus($orderNo,$admin_id = '',$note = '')
	{
		//获取订单信息
		...
		else if($orderRow['pay_status'] == 0)
		{
			...
			if($is_success == '')
			{
				...
				$fundObj  = new IModel('fund');
				$amountArray = array(
					'a_amount' => 'a_amount + '.$orderRow['order_amount'],
					'sup_count' => 'sup_count + 1',
					);
				$fundObj->setData($amountArray);
				$fundObj->update('id = '.$orderRow['fund_id']);
			}
